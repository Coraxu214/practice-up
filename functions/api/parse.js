// ============================================================
// EdgeOne Pages Function:POST /api/parse
// 路径:functions/api/parse.js → /api/parse(自动路由)
// 写法跟 Cloudflare Pages Functions 一致(Web Standards)。
// ------------------------------------------------------------
// 在 EdgeOne 控制台 → Pages 项目 → 设置 → 环境变量 配置:
...
// ---- 简易内存 IP 限频(边缘节点冷启动会重置,best-effort)----
...
export async function onRequestPost(context) {
  ...
  // 2. 邀请码(可选)
  const requiredCode = env.INVITE_CODE;
  if (requiredCode) {
  ...
  if (!text && !image) {
    return json({ error: '没有内容可解析' }, 400);
  }
  if (image && !text) {
    return json({ error: 'DeepSeek 暂不支持图片解析,请粘贴文字' }, 400);
  }
